window.onerror = function (message, source, lineno, colno, error) {
    __PS_MV_REG = [];
    return alert('Es ist ein Fehler aufgetreten! Melde ihn bitte dem Entwickler! ' + message + ' source: ' + source + ' lineno: ' + lineno + ' colno: ' + colno + ' error: ' + error);
};
...
function handleError(thrownError, showErrorPage) {
    if (thrownError === 'Authorization Required') {
        var name331 = $('#inputName').val(window.localStorage.name);
        window.localStorage.removeItem('name');
        __PS_MV_REG = [];
        return pushState('/login', { lastUrl : window.location.href, lastState : window.history.state })();
    } else {
        if (thrownError === 'Forbidden') {
            var errorMessage = 'Du hast nicht die benötigten Berechtigungen, um diese Aktion durchzuführen. Sag mir Bescheid, wenn du glaubst, dass dies ein Fehler ist.';
            ...
            var errorMessage332 = 'Unbekannter Fehler: ' + thrownError;
            if (showErrorPage) {
                $('#errorMessage').text(errorMessage332);
                __PS_MV_REG = [];
                return showTab('#error');
            } else {
                __PS_MV_REG = [];
                return alert(errorMessage332);
            };
        };
    };
};
function setFullscreen(value) {
    if (value && $('.fullscreen').length === 0) {
        ...
        __PS_MV_REG = [];
        return !value && $('.fullscreen').length === 1 ? $('article').summernote('fullscreen.toggle') : null;
    };
};
var finishedButton = function (context) {
    return $.summernote.ui.button({ contents : '<i class=\"fa fa-check\"/>',
                                    tooltip : 'Fertig',
                                    click : function () {
...
var cancelButton = function (context) {
    return $.summernote.ui.button({ contents : '<i class=\"fa fa-times\"/>',
                                    tooltip : 'Abbrechen',
                                    click : function () {
        __PS_MV_REG = [];
        return confirm('Möchtest du die Änderung wirklich verwerfen?') ? window.history.back() : null;
    }
                                  }).render();
};
var wikiLinkButton = function (context) {
    return $.summernote.ui.button({ contents : 'S',
...
function readCookie(name) {
    ...
    var _js334 = ca.length;
    for (var _js333 = 0; _js333 < _js334; _js333 += 1) {
        var c = ca[_js333];
...
$('#publish-changes').click(function () {
    ...
    tempDom.find('.formula').each(function () {
        return this.innerHTML = '\\( ' + MathLive.getOriginalContent(this) + ' \\)';
    });
    __PS_MV_REG = [];
    return $.post('/api' + articlePath, { summary : changeSummary,
                                       html : tempDom.html(),
...
function sendFile(file, editor, welEditable) {
    ...
    __PS_MV_REG = [];
    return window.fileUploadXhr = $.ajax({ data : data,
                                        type : 'POST',
                                        xhr : function () {
...
function progressHandlingFunction(e) {
    __PS_MV_REG = [];
    return e.lengthComputable ? $('#uploadProgress').css('width', 100 * (e.loaded / e.total) + '%') : null;
};
$('#uploadProgressModal').on('shown.bs.modal', function (e) {
    __PS_MV_REG = [];
    return window.fileUploadFinished ? $('#uploadProgressModal').modal('hide') : null;
});
    ...
    $('article').summernote({ lang : 'de-DE',
        ...
        return setTimeout(function () {
            return canCall = true;
                              ...
                              dialogsFade : true,
                              focus : true,
                              buttons : { finished : finishedButton,
                                          cancel : cancelButton,
                                          wikiLink : wikiLinkButton
                                        },
                              toolbar : [['style', ['style.p', 'style.h2', 'style.h3', 'superscript', 'subscript']], ['para', ['ul', 'ol', 'indent', 'outdent']], ['insert', ['link', 'picture', 'table', 'math']], ['management', ['undo', 'redo', 'finished']]],
                              cleaner : { action : 'both',
                                          newline : '<p><br></p>',
                                          notStyle : 'position:absolute;top:0;left:0;right:0',
                                          icon : '<i class=\"note-icon\">[Your Button]</i>',
                                          keepHtml : true,
                                          keepOnlyTabs : ['<h1>', '<h2>', '<h3>', '<h4>', '<h5>', '<h6>', '<p>', '<br>', '<ol>', '<ul>', '<li>', '<b>', '<strong>', '<i>', '<a>', '<sup>', '<sub>', '<img>'],
                                          keepClasses : false,
                              ...
                              popover : { math : ['math', ['edit-math', 'delete-math']],
...
$('.edit-button').click(function (e) {
    e.preventDefault();
    var pathname335 = window.location.pathname.split('/');
    pushState('/wiki/' + pathname335[2] + '/edit', window.history.state);
    __PS_MV_REG = [];
    return false;
});
$('#create-article').click(function (e) {
    e.preventDefault();
    var pathname336 = window.location.pathname.split('/');
    pushState('/wiki/' + pathname336[2] + '/create', window.history.state);
    __PS_MV_REG = [];
    return false;
});
$('#show-history').click(function (e) {
    e.preventDefault();
    var pathname337 = window.location.pathname.split('/');
    pushState('/wiki/' + pathname337[2] + '/history', window.history.state);
    __PS_MV_REG = [];
    return false;
});
function cleanup() {
...
function getUrlParameter(param) {
    var pageUrl338 = window.location.search.substring(1);
    var urlVariables = pageUrl.split('&');
    var _js340 = urlVariables.length;
    for (var _js339 = 0; _js339 < _js340; _js339 += 1) {
        var parameterName = urlVariables[_js339];
...
function handleLogout(path) {
    if ((results = (new RegExp('^/logout$')).exec(path)) != null) {
        ...
        showTab('#loading');
        __PS_MV_REG = [];
        return $.post('/api/logout', { csrf_token : readCookie('CSRF_TOEN') }, function (data) {
            window.localStorage.removeItem('name');
            __PS_MV_REG = [];
...
function handleLogin(path) {
    if ((results = (new RegExp('^/login$')).exec(path)) != null) {
        ...
        if ('undefined' !== typeof urlUsername && 'undefined' !== typeof urlPassword) {
...
function handleArticles(path) {
    if ((results = (new RegExp('^/articles$')).exec(path)) != null) {
        ...
        return $.get('/api/articles', function (data) {
            ...
            $('#articles-list').html('');
            var _js342 = data.length;
            for (var _js341 = 0; _js341 < _js342; _js341 += 1) {
                var page = data[_js341];
...
function handleWikiName(path) {
    if ((results = (new RegExp('^/wiki/([^/]*)$')).exec(path)) != null) {
        ...
        return $.get('/api/wiki/' + pathname[2], function (data) {
            ...
            __PS_MV_REG = [];
            return errorThrown === 'Not Found' ? showTab('#not-found') : handleError(errorThrown, true);
        });
    };
};
...
function handleWikiNameHistory(path) {
    if ((results = (new RegExp('^/wiki/([^/]*)/history$')).exec(path)) != null) {
        ...
        return $.get('/api/history/' + pathname[2], function (data) {
            $('#history-list').html('');
            var _js344 = data.length;
            for (var _js343 = 0; _js343 < _js344; _js343 += 1) {
                var page = data[_js343];
                ...
                template.find('.history-show').data('href', '/wiki/' + pathname[2] + '/history/' + page.id);
                template.find('.history-diff').data('href', '/wiki/' + pathname[2] + '/history/' + page.id + '/changes');
...
function handleWikiPageHistoryIdChanges(path) {
    if ((results = (new RegExp('^/wiki/([^/]*)/history/([^/]*)/changes$')).exec(path)) != null) {
        var page = results[1];
        var id = results[2];
        $('.edit-button').addClass('disabled');
        $('#currentVersionLink').data('href', '/wiki/' + page);
...
function handleQuizCreate(path) {
    if ((results = (new RegExp('^/quiz/create$')).exec(path)) != null) {
        showTab('#loading');
        __PS_MV_REG = [];
        return $.post('/api/quiz/create', { 'csrf_token' : readCookie('CSRF_TOKEN') }, function (data) {
            __PS_MV_REG = [];
...
function handleQuizIdResults(path) {
    if ((results = (new RegExp('^/quiz/([^/]*)/results$')).exec(path)) != null) {
        ...
        __PS_MV_REG = [];
        return $('#result').text('Du hast ' + window.correctResponses + ' Fragen richtig und ' + window.wrongResponses + ' Fragen falsch beantwortet. Das sind ' + ((window.correctResponses * 100) / (window.correctResponses + window.wrongResponses)).toFixed(1).toLocaleString() + ' %');
    };
};
$('.multiple-choice-submit-html').click(function () {
    var everythingCorrect = true;
    var i = 0;
    var _js345 = window.currentQuestion.responses;
    var _js347 = _js345.length;
    for (var _js346 = 0; _js346 < _js347; _js346 += 1) {
        var answer = _js345[_js346];
...
$('.next-question').click(function () {
    ...
    var pathname348 = window.location.pathname.split('/');
    __PS_MV_REG = [];
    return replaceState('/quiz/' + pathname348[2] + '/play/' + (parseInt(pathname348[4]) + 1));
});
$('#button-search').click(function () {
    ...
    return window.searchXhr = $.get('/api/search' + query, function (data) {
        ...
        if (data != null) {
            var _js350 = data.length;
            for (var _js349 = 0; _js349 < _js350; _js349 += 1) {
                var page = data[_js349];
                if (page.title === query) {
        ...
        __PS_MV_REG = [];
        return $('#search-results').stop().fadeIn();
    }).fail(function (jqXhr, textStatus, errorThrown) {
        __PS_MV_REG = [];
...
function loginPost(repeated) {
    __PS_MV_REG = [];
    return $.post('/api/login', { csrf_token : readCookie('CSRF_TOKEN'),
                                  name : name,
                                  password : password
                                }, function (data) {
        ...
        __PS_MV_REG = [];
        return window.history.state != null && 'undefined' !== typeof window.history.state.lastState && 'undefined' !== typeof window.history.state.lastUrl ? replaceState(window.history.state.lastUrl, window.history.state.lastState) : replaceState('/wiki/Hauptseite');
    }).fail(function (jqXhr, textStatus, errorThrown) {
        window.localStorage.removeItem('name');
        if (errorThrown === 'Forbidden') {
            if (repeated) {
                ...
                __PS_MV_REG = [];
                return $('#login-button').prop('disabled', false).html('Anmelden');
            } else {
                __PS_MV_REG = [];
...
$('body').on('click', '.add-response-possibility', function (e) {
    __PS_MV_REG = [];
    return $(this).siblings('.responses').append($($('#multiple-choice-response-possibility').html()));
});
$('.save-quiz').click(function () {
    var obj = new Object();
    var pathname351 = window.location.pathname.split('/');
    obj.questions = [];
    $('#questions').children().each(function () {
        ...
        __PS_MV_REG = [];
        return $(this).attr('class') === 'text-question' ? obj.questions.push(textQuestion($(this))) : null;
    });
    __PS_MV_REG = [];
    return $.post('/api/quiz' + pathname351[2], { csrf_token : readCookie('CSRF_TOKEN'), data : JSON.stringify(obj) }, function (data) {
        return window.history.replaceState(null, null, '/quiz/' + pathname351[2] + '/play');
    }).fail(function (jqXhr, textStatus, errorThrown) {
        __PS_MV_REG = [];
        return handleError(errorThrown, true);
    });
...
window.onbeforeunload = function () {
    var pathname352 = window.location.pathname.split('/');
    return pathname352.length === 4 && pathname352[1] === 'wiki' && (pathname352[3] === 'create' || pathname352[3] === 'edit') ? true : null;
};
