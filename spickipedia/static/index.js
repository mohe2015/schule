...
$(document).ready(function() {
  ...
  function handleError(thrownError, showErrorPage) {
    ...
    if (thrownError === 'Authorization Required') {
      ...
      window.history.pushState(
        { lastUrl: window.location.href, lastState: window.history.state },
        null,
        '/login'
      );
      updateState();
      ...
      var errorMessage =
        'Du hast nicht die benötigten Berechtigungen, um diese Aktion durchzuführen. Sag mit Bescheid, wenn du glaubst, dass dies ein Fehler ist.';
  ...
  function updateState() {
    ...
    if (pathname.length == 2 && pathname[1] == 'logout') {
      ...
      $.post('/api/logout', { csrf_token: readCookie('CSRF_TOKEN') }, function(
        data
      ) {
        window.localStorage.removeItem('name');
    ...
    if (pathname.length > 1 && pathname[1] == 'wiki') {
      ...
      if (pathname.length == 4 && pathname[3] == 'create') {
        ...
        if (
          ...
          window.history.state.content != null
        ) {
      ...
      if (pathname.length == 4 && pathname[3] == 'history') {
        ...
        var articlePath = window.location.pathname.substr(
          6,
          window.location.pathname.lastIndexOf('/') - 6
        );
        $.get('/api/history/' + articlePath, function(data) {
          ...
          for (var page of data) {
            ...
            t.find('.history-show').data(
              'href',
              '/wiki/' + articlePath + '/history/' + page.id
            );
            t.find('.history-diff').data(
              'href',
              '/wiki/' + articlePath + '/history/' + page.id + '/changes'
            );
    ...
    if (
      (pathname.length == 2 || pathname.length == 3) &&
      pathname[1] == 'search'
    ) {
      $('.edit-button').addClass('disabled');
      showTab('#search');
    ...
    $('#errorMessage').text(
      'Pfad nicht gefunden! Hast du dich vielleicht vertippt?'
    );
    showTab('#error');
  }
  ...
  $('.next-question').click(function() {
    ...
    window.history.replaceState(
      window.history.state,
      null,
      '/quiz/' + pathname[2] + '/play/' + (parseInt(pathname[4]) + 1)
    );
    updateState();
  });
  ...
  $('#login-form').on('submit', function(e) {
    ...
    $.post(
      ...
      function(data) {
        ...
        if (
          window.history.state !== null &&
          window.history.state.lastState !== undefined &&
          window.history.state.lastUrl !== undefined
        ) {
          window.history.replaceState(
  ...
  window.onpopstate = function(event) {
    if (window.lastURL) {
      ...
      if (
        (pathname.length == 4 && pathname[3] == 'create') ||
        (pathname.length == 4 && pathname[3] == 'edit')
      ) {
        if (confirm('Möchtest du die Änderung wirklich verwerfen?')) {
          updateState();
        }
