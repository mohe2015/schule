...
(defroute "/login" (chain ($ ".edit-button") (add-class "disabled"))
          (chain ($ "#publish-changes-modal") (modal "hide"))
          (let ((url-username (get-url-parameter "username"))
                (url-password (get-url-parameter "password")))
            (if (and (not (undefined url-username))
                     (not (undefined url-password)))
                (progn
                 (chain ($ "#inputName")
                  (val (decode-u-r-i-component url-username)))
                 (chain ($ "#inputPassword")
...
(chain ($ "#login-form")
 (on "submit"
  (lambda (e)
    ...
    (chain ($ "#login-button") (prop "disabled" t)
     (html
        ...
        (if (= (chain jq-xhr status) 403)
            ...
            (if (= (chain jq-xhr status) 400)
                (if repeated
                    (progn
                     (alert "Ungültige Zugangsdaten!")
                     (chain ($ "#login-button") (prop "disabled" f)
                      (html "Anmelden")))
                    (login-post t))
