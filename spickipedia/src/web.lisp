...
(my-defroute :get
  ...
  (let* ((article (find-dao 'wiki-article :title title)))
    (if (not article)
        (throw-code 404))
    (let ((revision
           (select-dao 'wiki-article-revision
...
(defroute ("/js/*" :method :get) (&key splat) (print (first splat))
          ...
          (with-cache (read-file-into-string
                       (merge-pathnames
                        (concatenate 'string "js/" (first splat))
                        *application-root*))
            (file-js-gen
             (concatenate 'string (namestring *application-root*) "js/"
                          (first splat))))) 
(my-defroute :get
    "/sw.lisp"
    nil
    ...
    "application/javascript"
  (with-cache (read-file-into-string
               (merge-pathnames "js/sw.lisp" *application-root*))
    (file-js-gen
