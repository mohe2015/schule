...
(my-defroute :post "/api/teachers" (:admin :user) (|name| |initial|)
 "text/html"
 (dbi:with-transaction *connection*
   (let* ((teacher (create-dao 'teacher))
          (revision
           (create-dao 'teacher-revision :author user :teacher teacher :name
            (first |name|) :initial (first |initial|))))
     (format nil "~a" (object-id teacher))))) 
(my-defroute :get "/api/teachers" (:admin :user) nil "application/json"
 (let* ((teachers (select-dao 'teacher))
        (teacher-revisions
         (mapcar
          #'(lambda (teacher)
              (first
               (select-dao 'teacher-revision (where (:= :teacher teacher))
                (order-by (:desc :id)) (limit 1))))
          teachers)))
   (encode-json-to-string (list-to-array teacher-revisions)))) 
(my-defroute :post "/api/courses" (:admin :user)
 (|subject| |type| |teacher| |is-tutorial| |topic|) "text/html"
 (dbi:with-transaction *connection*
   (let* ((course (create-dao 'course))
          (revision
           (create-dao 'course-revision :author user :course course :name
            (first |subject|) :initial (first |type|) :type (first |type|)
            :subject (first |subject|) :teacher
            (find-dao 'teacher :id (parse-integer (first |teacher|)))
            :is-tutorial (equal "on" (first |is-tutorial|)) :class
            (user-grade user) :topic (first |topic|))))
     (format nil "~a" (object-id course))))) 
(my-defroute :get "/api/courses" (:admin :user) nil "application/json"
 (if (user-grade user)
...
(my-defroute :get "/api/schedules" (:admin :user) nil "application/json"
...
(my-defroute :get "/api/schedule/:grade" (:admin :user) (grade)
 "application/json"
...
(my-defroute :post "/api/schedule/:grade/add" (:admin :user)
 (grade |weekday| |hour| |week-modulo| |course| |room|) "application/json"
 (dbi:with-transaction *connection*
   (let* ((schedule (find-dao 'schedule :grade grade))
          (last-revision
           (select-dao 'schedule-revision (where (:= :schedule schedule))
          ...
          (revision
           (create-dao 'schedule-revision :author user :schedule schedule))
          (data
           (create-dao 'schedule-data :schedule-revision revision :weekday
            (first |weekday|) :hour (first |hour|) :week-modulo
            (first |week-modulo|) :course
            (find-dao 'course :id (first |course|)) :room (first |room|))))
     (loop for old-data in (retrieve-dao 'schedule-data :schedule-revision
                            (car last-revision))
           do (create-dao 'schedule-data :schedule-revision revision :weekday
               (schedule-data-weekday old-data) :hour
...
(my-defroute :post "/api/schedule/:grade/delete" (:admin :user) (|id|)
 "application/json"
 (dbi:with-transaction *connection*
