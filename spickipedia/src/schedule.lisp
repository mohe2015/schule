...
(my-defroute :post
    "/api/courses"
    (:admin :user)
    (|subject| |type| |teacher| |is-tutorial| |topic|)
    "text/html"
  (dbi:with-transaction *connection*
    (let* ((course (create-dao 'course))
           (revision
            (create-dao 'course-revision :author user :course course :name
                        (first |subject|) :initial (first |type|) :type
                        (first |type|) :subject (first |subject|) :teacher
                        (find-dao 'teacher :id
                                  (parse-integer (first |teacher|)))
...
(my-defroute :post
    "/api/schedule/:grade/add"
    (:admin :user)
    (grade |weekday| |hour| |week-modulo| |course| |room|)
    "application/json"
  (dbi:with-transaction *connection*
    (let* ((schedule (find-dao 'schedule :grade grade))
           (last-revision
           ...
           (data
            (create-dao 'schedule-data :schedule-revision revision :weekday
                        ...
                        (find-dao 'course :id (first |course|)) :room
                        (first |room|))))
